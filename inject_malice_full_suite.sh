#!/bin/bash
# MALICE.OS™ Full Suite Injector — Phase VIII + IX
PLUGINS="$HOME/ScrapeForge/plugins"
mkdir -p "$PLUGINS"

# plugin_ai_trainer.py
cat <<'EOF' > "$PLUGINS/plugin_ai_trainer.py"
...
cat <<'EOF' > "$PLUGINS/plugin_ai_inferencer.py"
...
cat <<'EOF' > "$PLUGINS/plugin_ai_summarizer.py"
import sqlite3, os
db = os.path.expanduser("~/ScrapeForge/output/autonomous/autonomous.db")
conn = sqlite3.connect(db)
...
cat <<'EOF' > "$PLUGINS/plugin_ai_exporter.py"
...
cat <<'EOF' > "$PLUGINS/plugin_ai_dashboard.py"
import sqlite3, os
db = os.path.expanduser("~/ScrapeForge/output/autonomous/autonomous.db")
conn = sqlite3.connect(db)
...
cat <<'EOF' > "$PLUGINS/plugin_visual_loader.py"
echo "
╔══════════════════════════════╗
...
chmod +x "$PLUGINS"/plugin_*.py
echo "✅ MALICE.OS™ Full Suite Injected."
